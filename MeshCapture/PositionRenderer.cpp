...
class DisableImageUnRefVisitor : public osg::NodeVisitor
{
public:
		BBWrapper m_BB;

		DisableImageUnRefVisitor() :osg::NodeVisitor(TRAVERSE_ALL_CHILDREN) { setNodeMaskOverride(0xffffffff); }
...
public:
		virtual osgDB::ReaderWriter::ReadResult readNode(const std::string& fileName, const osgDB::ReaderWriter::Options* options)
		{
				...
				DisableImageUnRefVisitor visitor;
				if (result.getNode())
				{
...
public:
						...
						if (!texture2d)
						{
								...
								if (texture2d)
								{
										...
										if (img)
										{
												std::string filename = img->getFileName();
												filename = img->getFileName();
												if (!exists((m_outdir + filename).data()))
												{
														osgDB::writeImageFile(*img, m_outdir + filename);
												}
												//std::stringstream ss;
		...
		virtual NodeType getNodeType(osg::Node* node)
		{
				osg::Group* group = dynamic_cast<osg::Group*>(node);
				osg::PagedLOD* pageLOD = dynamic_cast<osg::PagedLOD*>(node);
				osg::Geode* geode = dynamic_cast<osg::Geode*>(node);
				...
				if (geode)
						return NodeType::Geode;
				if (pageLOD)
						return NodeType::PagedLOD;
				return NodeType::Group;
		}

		virtual void traverseGroup(osg::Node* node, int level = 0, bool isLeaf = false)
		{
				level = level + 1;
				if (m_BB.contains(node->getBound().center()))
				{				
						std::stringstream ss;
						ss << m_imageID++ << "_ljm.osg";
						//osgDB::writeNodeFile(*node, ss.str());
				}
				osg::Group* group = dynamic_cast<osg::Group*>(node);
				osg::PagedLOD* pageLOD = dynamic_cast<osg::PagedLOD*>(node);
				osg::Geode* geode = dynamic_cast<osg::Geode*>(node);
				...
				if (group && !geode)
				{
						...
						for (size_t i = 0; i < group->getNumChildren(); i++)
						{
								nodeTypes.push_back(getNodeType(group->getChild(i)));
								if (i == 0)
										first = nodeTypes[i];
								else if (first != nodeTypes[i])
								{
										isSame = false;
								}
						}
						isLeaf = isSame && first == NodeType::Geode;
						if (!isLeaf)
						{
								printf("");
						}
						...
						for (size_t i = 0; i < group->getNumChildren(); i++)
						{
								traverseGroup(group->getChild(i), level, isLeaf);
						}
						return;
				}
...
public:
		...
		void reset(osg::Camera* camera, osg::BoundingBoxd bb, double altAngle = 0, double azimuthAngle = 0)
		{
				//altAngle = 0;
				//bb = osg::BoundingBoxd(-100, -100, -100, 100, 100, 100);
				osg::Vec3d minBB(bb.xMin(), bb.yMin(), bb.zMin());
				...
				double ysize = (((cameraSpaceBB.center() - osg::Vec3d(0, cameraSpaceBB.yhalfsize(), 0)) * inverseViewMatrix) - ((cameraSpaceBB.center() + osg::Vec3d(0, cameraSpaceBB.yhalfsize(), 0)) * inverseViewMatrix)).length();
				double zsize = (((cameraSpaceBB.center() - osg::Vec3d(0, 0, cameraSpaceBB.zhalfsize())) * inverseViewMatrix) - ((cameraSpaceBB.center() + osg::Vec3d(0, 0, cameraSpaceBB.zhalfsize())) * inverseViewMatrix)).length();
				camera->setProjectionMatrixAsOrtho(-xsize * 0.5, xsize*0.5, -ysize * 0.5, ysize*0.5, 0, zsize * 3);
		}
...
#pragma endregion
	...
	//	viewer.setCameraManipulator(new osgGA::TrackballManipulator);
		...
		localbb = BBWrapper(transformedBB);
		orthoCamera->reset(localbb, altAngle, azimuthAngle);
		orthoCamera->reset(viewer.getCamera(), localbb, altAngle, azimuthAngle);
		viewer.frame();
		while (pager->getRequestsInProgress())
		{
				orthoCamera->reset(viewer.getCamera(), localbb, altAngle, azimuthAngle);
		...
		geomVisitor.save("E:/Code/TestMesh/TestMesh.osgb");
