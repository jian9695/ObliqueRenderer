...
int main(int argc, char** argv)
{
		...
		//tex->setSourceType(GL_FLOAT);
		osg::ref_ptr<osg::Image> img = new osg::Image;
		img->allocateImage(texSizeX, texSizeY, 1, GL_ALPHA, GL_FLOAT);
		...
		osg::StateSet* stateset = orthoCamera->getOrCreateStateSet();
		osg::ref_ptr<osg::Program> program = new osg::Program;
		char vertexShaderSource[] =
		...
		program->addShader(new osg::Shader(osg::Shader::VERTEX, vertexShaderSource));
		stateset->setAttribute(program.get(), osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE);
		osg::Matrix matViewInverse = osg::Matrix::identity();
		stateset->addUniform(new osg::Uniform(osg::Uniform::FLOAT_MAT4, "viewMatrixInverse"));
		osg::ClampColor* clamp = new osg::ClampColor();
		clamp->setClampVertexColor(GL_FALSE);
		clamp->setClampFragmentColor(GL_FALSE);
		clamp->setClampReadColor(GL_FALSE);
		...
		for (int irow = 0; irow < orthoCamera->_nrows; irow++)
		{
				for (int icol = 0; icol < orthoCamera->_ncols; icol++)
				{
						...
						//	continue;
						orthoCamera->setupCameraAtCell(irow, icol);
						matViewInverse =	orthoCamera->getViewMatrix();
						matViewInverse.invert(orthoCamera->getViewMatrix());
						stateset->getUniform("viewMatrixInverse")->set(matViewInverse);
						viewer.frame();
						stateset->getUniform("viewMatrixInverse")->set(matViewInverse);
						databasePager->frame();
						while (databasePager->getFileRequestListSize() > 0)
