...
public:
	...
	void setupCameraAtPosition(osg::Vec3d pos)
	{
		...
		line->set(l1, l2);
		osg::Vec3d p1 = _center + (osg::Vec3d(_bb.xMin(), _bb.yMin(), _bb.zMin()) - _center) * 10;
		osg::Vec3d p2 = _center + (osg::Vec3d(_center.x(), _bb.yMax(), _bb.zMin()) - _center) * 10;
		osg::Vec3d p3 = _center + (osg::Vec3d(_bb.xMax(), _bb.yMin(), _bb.zMin()) - _center) * 10;
		double ratio;
		...
		int ncol = (int)((pos.y() - _bb.yMin()) / _cellsize);
		...
		this->setProjectionMatrixAsOrtho(left - _cellsize, right + _cellsize, bottom - _cellsize, top + _cellsize, _znear, _zfar);
