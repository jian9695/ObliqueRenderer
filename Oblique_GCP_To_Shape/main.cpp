...
void bindShader(osg::StateSet* stateset)
{
	osg::ref_ptr<osg::Program> program = new osg::Program;
...
int main(int argc, char** argv)
{
	...
	bindShader(node->getOrCreateStateSet());
	...
	viewer.getCamera()->setClearColor(osg::Vec4(0, 1, 0, 1));
	...
	orthoCamera->setClearColor(osg::Vec4(0, 1, 0, 1));
	...
	ShapeFile outputshape;
	outputshape.create("B:/VGE/DSMRenderer_2016_02_14/bin/Weihai25cm/45_135_GCP.shp");
	OGRFeatureDefn *poFDefn = outputshape.poLayer->GetLayerDefn();
	OGRFieldDefn defid("id", OGRFieldType::OFTInteger);
	outputshape.poLayer->CreateField(&defid);
	...
	while ((poFeature = shp.poLayer->GetNextFeature()) != NULL)
	{
		...
		//if (pos2.x() < bb.xMin() || pos2.x() > bb.xMax() || pos2.y() < bb.yMin() || pos2.y() > bb.yMax())
		//{
		...
		if (row > -1 && col > -1)
		{
			if (orthoCamera->findPixelPosition(img, destPos))
			{
				...
				OGRFeature* newFea = OGRFeature::CreateFeature(outputshape.poLayer->GetLayerDefn());
				OGRPoint po;
				po.setX(destPos.x());
				po.setY(destPos.y());
				newFea->SetGeometry(&po);
				newFea->SetField(0, pointid);
				outputshape.poLayer->CreateFeature(newFea);
				OGRFeature::DestroyFeature(newFea);
				//orthoCamera->produceImage(row, col, img, "./test2/");
				//found = true;
