...
int main(int argc, char** argv)
{
	...
	if (argc > 5)
	{
		xmin = atof(argv[5]); xmax = atof(argv[6]);
		ymin = atof(argv[7]); ymax = atof(argv[8]);
		boundSpecified = true;
	}
	...
	double azimuthAngleStep = 45;
	std::vector<ObliqueAngle> angles;
	angles.push_back(ObliqueAngle(90, 0));
	while (azimuthAngle < 360)
	{
		angles.push_back(ObliqueAngle(altAngle, azimuthAngle));
		azimuthAngle += azimuthAngleStep;
	}
	std::string temp_outdir = (qoutdir.absolutePath() + "/" + "tmp" + "/").toLocal8Bit().data();
	QDir(temp_outdir.data()).mkpath(".");
	for (size_t i = 0; i < angles.size(); i++)
	{
		ObliqueAngle obliqueAngle = angles[i];
