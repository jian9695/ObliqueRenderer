...
public:
		...
		void crop(OGREnvelope bb, std::string outfile)
		{
				...
				std::tie(startCol, startRow, startIndex) = getCellIndex(bb.MinX, bb.MaxY);
				std::tie(endCol, endRow, endIndex) = getCellIndex(bb.MaxX, bb.MinY);
				if (startCol < 0)
						startCol = 0;
				if (startRow < 0)
						startRow = 0;
				...
				newdt->adfGeoTransform[3] = bb.MaxY;
				//newdt->bound = destBB;
				newdt->numbands = 1;
