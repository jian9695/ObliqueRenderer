...
SolarRadiation GrassSolar::calculateSolarRadiation(SolarParam& solar_param, ShadowCaster* shadowCaster, osg::Vec3d pos)
{
		std::vector<SunVector> sunVectors = getSunVectors(solar_param);
		std::vector<osg::Vec3d> lightDirs = sunVector2LightDir(sunVectors);
		...
		for (long i = 0; i < lightDirs.size(); i++)
		{
				...
				if (shadowCaster->Intersects(r, t, slope, aspect))
				{
						solar_param.shadowInfo[i] = true;
						shadowMasks += "1";
				}
...
void GrassSolar::calculateSolarRadiation(SolarParam& solar_param, osg::Node* sceneNode, ShadowCaster* shadowCaster, osg::Vec3d pos, std::vector<osg::Vec3>& rays)
{
		std::vector<SunVector> sunVectors = getSunVectors(solar_param);
		std::vector<osg::Vec3d> lightDirs = sunVector2LightDir(sunVectors);
		...
		for (long i = 0; i < lightDirs.size(); i++)
		{
				...
				Ray r(start, end - start);
				double t, aspect, slope;
				bool intersect3D = intersector->containsIntersections();
				bool intersect2D = shadowCaster->Intersects(r, t, slope, aspect);
				if (intersect3D != intersect2D && !intersect3D)
				{
						osg::Vec3d intersectPos3D = intersector->getFirstIntersection().getWorldIntersectPoint();
						osg::Vec3d intersectPos2D = start + r.dir * t;
						PrintVec3d(start);
						PrintVec3d(lightDirs[i]);
						PrintVec3d(intersectPos3D);
						PrintVec3d(intersectPos2D);
						rays.push_back(start);
						rays.push_back(pos + lightDirs[i] * 500);
				}
...
//std::vector<SolarRadiation> calculateSolarRadiation(SolarParam& solar_param, osg::Node* sceneNode, std::vector<osgUtil::LineSegmentIntersector::Intersection>& intersections)
//{
//		std::vector<SolarRadiation> rads;
//		for each (osgUtil::LineSegmentIntersector::Intersection intersection in intersections)
//		{
//				rads.push_back(calculateSolarRadiation(solar_param, sceneNode, intersection));
//		}
//}